...
import { X, MessageSquare, Tags, Binary, Search, Zap, Rocket } from 'lucide-react';
import {
  Instance,
  InstanceConfig,
  getBackendType,
...
const BackendIcon = ({ config }: { config: InstanceConfig }) => {
  ...
  switch (backendType) {
    ...
    case 'vllm':
      return <Rocket size={18} className="text-nord-12" />;
    default:
...
export function EditInstanceModal({ instance, hostId, onClose, onUpdate }: EditInstanceModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    } else if (isVllmConfig(formData)) {
      if (!(formData as VllmConfig).model_path || !formData.alias) {
        alert('Model Path and Alias are required');
        return;
      }
    ...
    if (isLlamaCppConfig(formData) || isSglangConfig(formData) || isVllmConfig(formData)) {
            ...
            {isVllmConfig(formData) ? (
              ...
              <>
                ...
                <div className="md:col-span-2">
                  ...
                  <select
                    name="reasoning"
                    value={(formData as LlamaCppConfig).reasoning || ''}
                    onChange={handleChange}
                    className="w-full px-3 py-2 bg-nord-2 border border-nord-3 text-nord-6 rounded-md focus:ring-2 focus:ring-nord-10 focus:border-transparent"
                  >
                ...
                <div>
                  ...
                  <input
                    type="text"
                    name="devices"
                    value={(formData as LlamaCppConfig).devices || ''}
                    onChange={handleChange}
                ...
                <div>
                  ...
                  <select
                    name="split_mode"
                    value={(formData as LlamaCppConfig).split_mode || ''}
                    onChange={handleChange}
                    className="w-full px-3 py-2 bg-nord-2 border border-nord-3 text-nord-6 rounded-md focus:ring-2 focus:ring-nord-10 focus:border-transparent"
                  >
                ...
                <div>
                  ...
                  <input
                    type="text"
                    name="tensor_split"
                    value={(formData as LlamaCppConfig).tensor_split || ''}
                    onChange={handleChange}
                    placeholder="3,1"
                    className="w-full px-3 py-2 bg-nord-2 border border-nord-3 text-nord-6 placeholder-nord-4 placeholder:opacity-60 rounded-md focus:ring-2 focus:ring-nord-10 focus:border-transparent font-mono text-sm"
                  />
                ...
                <div>
                  ...
                  <input
                    type="number"
                    name="main_gpu"
                    value={(formData as LlamaCppConfig).main_gpu ?? ''}
                    onChange={handleChange}
