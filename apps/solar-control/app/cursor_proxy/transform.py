...
from .aliases import is_cursor_alias
            ...
            if not isinstance(reasoning, str):
                ...
                if needs_reasoning and store is not None:
                    for lookup_key in lookup_keys:
                        restored = await store.get(str(lookup_key["key"]))
                        if restored is not None:
                            ...
                            if not lookup_key.get("portable"):
                                await store.backfill_portable_aliases(
                                    normalized,
                                    restored,
                                    cache_namespace,
                                    prior_messages,
...
async def normalize_messages(
    ...
    for message in messages:
        normalized, patched, missing, diagnostic = await normalize_message(
            message,
            store,
            normalized_messages,
            cache_namespace,
...
def upstream_model_for(original_model: str, config: ProxyConfig) -> str:
    # Every /cursor alias maps to the single configured upstream model; the
    # alias only selects the reasoning effort (handled by the router).
    if is_cursor_alias(original_model) or original_model == config.upstream_model:
        return config.upstream_model
    LOG.warning(
        "rewriting unknown model %r to configured upstream %r",
        original_model,
...
async def prepare_upstream_request(
    ...
    pre_repair_messages, _, _, _ = await normalize_messages(
...
async def record_response_reasoning(
...
async def rewrite_response_body(
    body: bytes,
    original_model: str,
    store: ReasoningStore | None,
    request_messages: list[dict[str, Any]],
    ...
    if isinstance(response_payload, dict):
        ...
        await record_response_reasoning(
            response_payload,
            store,
            request_messages,
            cache_namespace,
